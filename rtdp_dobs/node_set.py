...
class Node:
    ...
    def generate_key(px, py, vx, vy):
        return "%03d" % px + "%03d" % py + "%02d" % vx + "%02d" % vy
    ...
    def connect_to_graph(self, grid):
        for u in ACTION_SPACE:
            self.next_node.append(self.control(u[0], u[1], grid))
    ...
    @staticmethod
    def velocity_constraints(vx, vy):  # TODO max_vel
        return np.sign(vx) * min(abs(vx), max_vel), np.sign(vy) * min(abs(vy), max_vel)
    ...
    def control(self, ux, uy, grid):
        ...
        if status == FREE:
            ...
            start_p = STA_P
            return self.generate_key(start_p[0], start_p[1], 0, 0)
